...
class TestGetByPeriod:
    ...
    async def test_budget_partial_overlap(
        ...
        crossing = await budget_repository.add(
            Budget(
        ...
        june = await budget_repository.get_by_period(user.id, date(2026, 6, 1), date(2026, 6, 30))
        assert crossing.id in {b.id for b in june}

        july = await budget_repository.get_by_period(user.id, date(2026, 7, 1), date(2026, 7, 31))
    ...
    async def test_ordered_by_start_date(
        ...
        b_mar = await budget_repository.add(
            Budget(
        ...
        b_jan = await budget_repository.add(
            Budget(
                amount=Decimal("100.00"),
...
class TestGetByPeriodEdges:
    async def test_active_on_first_day_of_budget(
        ...
        result = await budget_repository.get_by_period(user.id, date(2026, 7, 1), date(2026, 7, 1))
        assert budget.id in {b.id for b in result}
    ...
    async def test_not_active_day_after_end(
        ...
        result = await budget_repository.get_by_period(user.id, date(2026, 8, 1), date(2026, 8, 1))
        assert budget.id not in {b.id for b in result}
    ...
    async def test_single_day_budget(
        ...
        one_day = await budget_repository.add(
            Budget(
                amount=Decimal("100.00"),
                currency_code=uah_currency.code,
                category_id=category.id,
    ...
    async def test_budget_wider_than_search_period(
        ...
        yearly = await budget_repository.add(
            Budget(
